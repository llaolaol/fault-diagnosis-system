...
export class LogicGateRenderer {
  ...
  private adjustConfigForViewport(): void {
    ...
    if (viewportWidth < 768) {
      ...
      this.config.verticalSpacing = 160
    } else if (viewportWidth < 1200) {
      // 平板设备：中等间距
      this.config.horizontalSpacing = 250
      this.config.levelHeight = 220
      this.config.verticalSpacing = 170
    } else {
      // 桌面设备：标准间距
      this.config.horizontalSpacing = 280
      this.config.levelHeight = 250
      this.config.verticalSpacing = 180
    }
  }
  ...
  private calculateHierarchicalLayout(faultTree: EnhancedFaultTreeNode): void {
    // 如果是虚拟根节点，直接从其子节点开始布局
    if (faultTree.type === 'virtual_root' && faultTree.children) {
      // 将虚拟根节点的子节点作为顶级节点进行布局
      const levelNodes = new Map<number, EnhancedFaultTreeNode[]>()
      levelNodes.set(0, faultTree.children)
      
      // 继续计算子节点的层级
      faultTree.children.forEach(child => {
        const childLevelNodes = this.groupNodesByLevel(child, 1)
        childLevelNodes.forEach((nodes, level) => {
          if (!levelNodes.has(level)) {
            levelNodes.set(level, [])
          }
          levelNodes.get(level)!.push(...nodes)
        })
      })
  ...
  private generateConnections(node: EnhancedFaultTreeNode): void {
    const traverse = (currentNode: EnhancedFaultTreeNode) => {
      if (currentNode.children) {
        currentNode.children.forEach(child => {
          // 跳过虚拟根节点的连线
          if (currentNode.type !== 'virtual_root') {
  ...
  private renderNodes(node: EnhancedFaultTreeNode, positions: Map<string, RenderPosition>, bounds: any): string {
    ...
    const traverse = (currentNode: EnhancedFaultTreeNode) => {
      ...
      // 跳过虚拟根节点的渲染
      if (pos && currentNode.type !== 'virtual_root') {
