  ...
  <div class="fault-tree-management">
                ...
                <el-row :gutter="16" align="middle">
                  <el-col :span="16">
                    <el-button-group>
                      ...
                      <el-button :icon="Plus" @click="addChildNode" :disabled="!selectedNodeId">
                        添加子节点
                      </el-button>
                      <el-button :icon="Plus" @click="addSiblingNode" :disabled="!selectedNodeId || isRootNode">
                        添加兄弟节点
                      </el-button>
                  ...
                  <el-col :span="8">
                    <div v-if="selectedNodeId" class="selected-node-info">
                      <el-tag type="info" size="small">
                        已选择: {{ getSelectedNodeName() }}
                      </el-tag>
                    </div>
                    <div v-else class="selected-node-info">
                      <el-text type="info" size="small">请先选择一个节点</el-text>
                    </div>
                  </el-col>
                </el-row>
              </div>
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="nodeEditMode === 'create' && currentEditNode.parent_id" label="父节点" prop="parent_id">
          <el-input :value="getParentNodeName()" disabled placeholder="父节点" />
        </el-form-item>
        ...
        <el-form-item v-if="currentEditNode.type === 'logic_gate'" label="判断条件" prop="condition">
          <el-input
            ...
            type="textarea"
            :rows="2"
            placeholder="例如: H2_ppm > 150 AND CH4_ppm > 50&#10;支持的操作符: >, <, >=, <=, ==, !=&#10;支持的逻辑: AND, OR, NOT"
          />
          <div class="condition-helper">
            <el-text type="info" size="small">
              常用参数: H2_ppm, CH4_ppm, C2H6_ppm, C2H4_ppm, C2H2_ppm, CO_ppm, CO2_ppm
            </el-text>
          </div>
        </el-form-item>

        <el-form-item v-if="currentEditNode.type === 'logic_gate'" label="逻辑门状态" prop="state">
          <el-radio-group v-model="currentEditNode.state">
...
const isRootNode = computed(() => {
  return selectedNodeId.value === currentEditTree.value?.tree?.id
})

const getSelectedNodeName = () => {
  if (!selectedNodeId.value || !currentEditTree.value?.tree) return ''
  
  const selectedNode = findNodeById(currentEditTree.value.tree, selectedNodeId.value)
  return selectedNode ? selectedNode.name : ''
}

const getParentNodeName = () => {
  if (!currentEditNode.value?.parent_id || !currentEditTree.value?.tree) return ''
  
  const parentNode = findNodeById(currentEditTree.value.tree, currentEditNode.value.parent_id)
...
const handlePreviewNodeClick = (node: EnhancedFaultTreeNode) => {
...
// 节点编辑相关方法
const handleEditTreeNodeClick = (node: EnhancedFaultTreeNode) => {
...
const addSiblingNode = () => {
  ...
  // 找到父节点ID
  const parentNodeId = findParentNodeId(currentEditTree.value.tree, selectedNodeId.value)
  if (!parentNodeId) {
    ElMessage.error('根节点无法添加兄弟节点')
    return
  }
...
const handleSaveNode = async () => {
  ...
  if (nodeEditMode.value === 'create') {
    if (!currentEditTree.value.tree) {
      ...
      // 添加子节点或兄弟节点
      const newNode = currentEditNode.value as EnhancedFaultTreeNode
      
      if (newNode.parent_id) {
        if (newNode.parent_id === currentEditTree.value.tree.id) {
          // 添加到根节点下
          if (!currentEditTree.value.tree.children) {
            currentEditTree.value.tree.children = []
          }
          currentEditTree.value.tree.children.push(newNode)
        } else {
          // 添加到其他节点下
          const parentNode = findNodeById(currentEditTree.value.tree, newNode.parent_id)
          if (parentNode) {
...
const findParentNodeId = (node: EnhancedFaultTreeNode, targetId: string): string | null => {
  if (node.children) {
    for (const child of node.children) {
      if (child.id === targetId) {
        return node.id
      }
      const parentId = findParentNodeId(child, targetId)
